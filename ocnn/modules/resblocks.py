...
from ocnn.modules import (Conv1x1BnRelu, OctreeConvBnRelu, Conv1x1Bn,
                          ...
                          OctreeConvGn, Conv1x1RnRelu, Conv1x1Rn,
                          OctreeConvRnRelu, OctreeConvRn,
                          OctreeDeconvRnRelu)
...
class OctreeResBlockRn(torch.nn.Module):
  ...
  def __init__(self, in_channels: int, out_channels: int, stride: int = 1,
    ...
    if self.stride == 2:
      self.maxpool = OctreeMaxPool(self.depth)
    self.conv3x3a = OctreeConvRnRelu(in_channels, channelb, nempty=nempty)
    self.conv3x3b = OctreeConvRn(channelb, out_channels, nempty=nempty)
    if self.in_channels != self.out_channels:
      self.conv1x1 = Conv1x1Rn(in_channels, out_channels)
    self.relu = torch.nn.ReLU(inplace=True)
